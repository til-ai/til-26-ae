...
def _los_to_tile(
    ...
    """
    gosh i love python for loops
    Return True if (tx, ty) is reachable from (ox, oy) with no wall crossing.
    """
    if tx == ox and ty == oy:
        return True
    path = supercover_line(np.array([ox, oy]), np.array([tx, ty]))
