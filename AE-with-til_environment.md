...
- [Use during training](#use-during-training)
  - [Writing an environment wrapper](#writing-an-environment-wrapper)
  - [Observation shaping and persistence](#observation-shaping-and-persistence)
...
We've significantly expanded the observation space from last year. This time, we give a series of binary channels (with significant sparsity, i.e many 0s, few 1s, per channel). For a full description of what each channel represents, see [observation.py](til_environment/observation.py).
...
| 2 | `LEFT` | Move one cell to the left |
...
| `tile_respawn_steps` | 30 | Steps before a collected tile reappears |
...
| `freeze_turns` | 3 | Rounds the agent is frozen after reaching 0 HP |
...
| `blast_radius` | 2 | Manhattan radius of explosion |
| `timer` | 4 | Rounds until detonation |
...
| `attack_damage` | 1.0 | Attacker (×damage dealt) | Bomb damages an agent or base; defender receives the equal-and-opposite penalty automatically |
...
| `truncation` | 0 | Agent | Episode ends by step limit |
...
Custom wrappers inherit from `BaseWrapper` in PettingZoo. You may want to write custom wrappers to modify the way observations are returned. A basic form of wrapping can be observed in the [basic_env function](til_environment/bomberman_env.py)
...
├── actions.py             # Action enum, ActionMask (legal-move computation)
...
├── test.py                # Interactive harness: launch with `uv run python -m til_environment.test`
│
├── entities/
...
**Death and respawn.** When an agent's HP reaches 0 it enters a *frozen* state for `freeze_turns` steps — it stays on the grid but its entire action mask is zeroed. After the freeze it respawns at the same tile at full HP. Agents are never removed from the game; only base destruction or step truncation ends an episode.

**Resource economy.** Each team has a shared resource pool (`team_resources`) and bomb stockpile (`team_bombs`). Resources accumulate passively every step at a fixed rate (`base_resource_rate`), and are also collected by stepping on resource tiles. Whenever `team_resources ≥ bomb_cost` the surplus auto-converts into bombs.

**Bombs.** A placed bomb counts down for `timer` steps then detonates in a cross-shaped blast of Manhattan radius `blast_radius`. The blast damages agents, bases, and destroys destructible walls in range. Structural (maze) walls block the blast.

**Observation channels.** The viewcone tensor has 25 channels encoding visibility, wall edges (structural and destructible, per direction), tile types, ally/enemy entities, bomb presence and timers, and HP levels. Once more, see [`observation.py`](til_environment/observation.py) (`ViewChannel` enum) for the full list.
